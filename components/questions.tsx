...
const Questions = ({ questions, limit, category }: Props) => {
  ...
  return (
    <div className="bg-white px-3 py-5 md:p-6 shadow-xl w-full md:w-[80%] lg:w-[70%] max-w-5xl sm:rounded-lg border">
      ...
      <div className="w-full">
        ...
        <div className="py-4 md:py-5 xl:py-7 grid grid-cols-2 gap-4">
          {answers.map((answer, i) => (
            <button
              key={i}
              className={`option ${selected && handleSelect(answer)} min-h-24`}
              disabled={!!selected}
        ...
        {/* <Separator /> */}
        <div className="flex mt-5 md:justify-between md:flex-row flex-col gap-4 mx-auto max-w-xs w-full">
          <Button
            disabled={!selected}
            onClick={() =>
          ...
          <Button disabled={true} variant={"secondary"} onClick={handleQuit}>
            Important
          </Button>
